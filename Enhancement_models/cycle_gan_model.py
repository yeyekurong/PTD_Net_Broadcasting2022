...
class CycleGANModel(BaseModel):
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            ...
            self.loss_idt_B = self.criterionIdt(self.idt_B, self.real_A) * lambda_A * lambda_idt #the samll lambda_idt will result the distortion for clear images
        else:
            self.loss_idt_A = 0
            self.loss_idt_B = 0
        ...
        self.loss_G_B = 0.5*(self.criterionGAN(self.netD_B(self.fake_A), True) + self.criterionGAN(self.netD_B(fake_A_sample), True))
        # Forward cycle loss || G_B(G_A(A)) - A||
        self.loss_cycle_A = self.criterionCycle(self.rec_A, self.real_A) * lambda_A
        # Backward cycle loss || G_A(G_B(B)) - B||
        self.loss_cycle_B = self.criterionCycle(self.rec_B, self.real_B) * lambda_B
        # combined loss and calculate gradients
        m_time = (lambda_B * 1) #EBP
        self.loss_trans_B = self.criterionTrans2(self.fake_A, self.real_B) * m_time + self.criterionTrans1(self.fake_A.max(1)[0], self.real_B.max(1)[0]) * (m_time/4)
        #self.loss_trans_B = self.criterionTrans2(self.fake_A.max(1)[0], self.real_B.max(1)[0]) * (m_time/5)
        #self.loss_DC = 0.1 * DCLoss(self.fake_A, 25) 
        self.loss_color = 10 * ColorLoss(self.fake_A)
        #self.loss_entrophy = 1 * EntropyLoss(self.fake_A)
        
        self.loss_grad = self.GradLoss(self.fake_A) * 2
        self.loss_grad = self.loss_grad.mean()
        self.loss_TV = L1_TVLoss(self.fake_A-self.real_B) * float(10) #2.5 TV
        
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B + self.loss_grad + self.loss_TV + self.loss_color + self.loss_trans_B
    ...
    def optimize_parameters(self, critic_iters, init_epoch, refine_epoch, load_path=''):
        """Calculate losses, gradients, and update network weights; called in every training iteration"""
                
        self.forward(critic_iters, init_epoch)      # compute fake images and reconstruction images.
